...
exports.compareFiles = function(fileSetA = [], fileSetB = [], compareSettings = []) {
    ...
    collisionChecked.collisions.forEach(collision => collisionChecked.unique.push(compareArray(collision, compareSettings))); //Check each collision and then return the primary.
    return collisionChecked.unique;
};

exports.renameMerge = function(fileSetA = [], fileSetB = [], renameSetting = 'num') {
    ...
    let collisionChecked = findCollisions(fileSetA, fileSetB);
    collisionChecked.collisions.forEach(collision => collisionChecked.unique = [...collisionChecked.unique, ...renameArray(collision, renameSetting)])
    return collisionChecked.unique;
};
...
function renameArray(collision, renameSetting) {
    switch(renameSetting) {
        case 'date': //Renames with current datetime.
            ...
            collision[1].renamed = `${collision[1].fileName.replace(/\.[^/.]+$/, "")} - ${currentDateTime.getFullYear()}.${currentDateTime.getMonth()+1 /* 0 indexed */}.${currentDateTime.getDate()}-${currentDateTime.getHours()}.${currentDateTime.getMinutes()}${path.extname(collision[1].fileName)}`;
            break;
        case 'num': //Appends '(1)' to filename.
            collision[1].renamed = `${collision[1].fileName.replace(/\.[^/.]+$/, "")} (1)${path.extname(collision[1].fileName)}`; 
            break;
        case 'title': //Appends name of merged parent folder to title. If both merge folders have the same name, appends a num to parent folder name.
            ...
            collision[0].renamed = `${collision[0].fileName.replace(/\.[^/.]+$/, "")} - ${titleOne}${path.extname(collision[0].fileName)}`;
            collision[1].renamed = `${collision[1].fileName.replace(/\.[^/.]+$/, "")} - ${titleTwo}${path.extname(collision[1].fileName)}`;
            break;
    }
    return collision;
};