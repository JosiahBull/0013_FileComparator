...
function checkDir(dirPath, create = true) { //Returns true/false bool, if create flag is true will create the dir.
    if (!fs.existsSync(dirPath) && !createdDirectories.includes(dirPath)) { //Check that file exists and that it hasn't already been created by another thread.
        ...
        return mkDirProm(dirPath, {recursive: true}).then(() => true).catch((err) => {console.log(`Error when creating dir: ${dirPath}`); throw err}); //After creation returns true.
    }
