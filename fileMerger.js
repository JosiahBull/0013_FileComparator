...
function mergeFiles(fileList = [], outputDir = '') {
    ...
    return Promise.all(fileList.map(file => dirCheck.check(path.join(outputDir, file.relativePath)))).then(() => { //Check that the full directory structure has been created.
        return fileList.map(file => copyFileProm(path.join(file.path, file.fileName), path.join(outputDir, file.relativePath, (file.renamed.length > 0) ? file.renamed : file.fileName)).then(() => { //Copy each file.
            console.log(`File: ${file.fileName} copied from ${file.path} to ${outputDir} succesfully.`);
        }).catch(err => {
            console.log(`\n\n${err}\nFile: ${JSON.stringify(file, 2, null)}\nOutput file: ${path.join(outputDir, file.relativePath, (file.renamed.length > 0) ? file.renamed : file.fileName)}`)
            throw err;
        }));
    });
};
